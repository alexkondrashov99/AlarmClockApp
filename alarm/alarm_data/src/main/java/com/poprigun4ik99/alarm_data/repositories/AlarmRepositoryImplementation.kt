...
class AlarmRepositoryImplementation(private val alarmDao: AlarmDao): AlarmRepository {
    ...
    override suspend fun markAlarmAsPassed(id: Long) {
        return withContext(DefaultDispatcherProvider.io()) {
            alarmDao.markAlarmRecordAsPassed(id)
        }
    }
    ...
    override suspend fun getAlarmById(id: Long): AlarmRecord {
        return withContext(DefaultDispatcherProvider.io()) {
            alarmDao.getAlarmRecordById(id)
        }
    }
